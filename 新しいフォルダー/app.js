...
function initEventListeners() {
    ...
    document.getElementById("form-event").addEventListener("submit", (e) => {
        ...
        // 🆕 バックグラウンド通知 (10分前) の登録処理を呼び出し
        registerBackgroundNotification(newEvent);

        closeModal("modal-event-edit"); //[cite: 2]
        renderCalendar(); //[cite: 2]
        renderDashboard(); //[cite: 2]
    });
...
// 🆕 バックグラウンド通知登録用ブリッジ関数
// ==========================================
function registerBackgroundNotification(eventItem) {
    // 通知権限がない、または終日予定の場合はスキップ[cite: 2]
    if (!("Notification" in window) || Notification.permission !== "granted" || eventItem.allDay) {
        return;
    }

    const eventTime = new Date(eventItem.start).getTime();
    const triggerTime = eventTime - (10 * 60 * 1000); // 10分前 (ミリ秒)

    // すでに10分前の時刻を過ぎている場合はスケジュールしない
    if (triggerTime <= Date.now()) {
        return;
    }

    // Service Worker に通知スケジュール情報を送信
    if ('serviceWorker' in navigator && navigator.serviceWorker.controller) {
        navigator.serviceWorker.controller.postMessage({
            ...
            payload: {
                ...
                body: `10分後に予定「${eventItem.title}」が始まります。`
            }
        });
    }
}